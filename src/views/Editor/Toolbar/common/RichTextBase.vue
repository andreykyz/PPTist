  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" style="width: 30%;">
        ...
        <TextColorButton first v-tooltip="t('richText.textColor')" :color="richTextAttrs.color">
      ...
      <Popover trigger="click" style="width: 30%;">
        ...
        <TextColorButton v-tooltip="t('richText.textHighlight')" :color="richTextAttrs.backcolor">
        ...
        <Button 
          class="font-size-btn"
          style="width: 20%;"
          v-tooltip="t('richText.fontSizeAdd')"
      ...
      <Button
        ...
        class="font-size-btn"
        style="width: 20%;"
        v-tooltip="t('richText.fontSizeReduce')"
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('美化改写')">{{ t('richText.aiRewrite.beautiful') }}</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('扩写丰富')">{{ t('richText.aiRewrite.expand') }}</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('精简提炼')">{{ t('richText.aiRewrite.concis') }}</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
          v-tooltip="t('richText.aiAssist')"
        ><span :class="{ 'ai-loading': isAIWriting }">{{ isAIWriting ? '' : 'AI' }}</span></CheckboxButton>
      </Popover>
      <CheckboxButton
        style="flex: 1;"
        v-tooltip="t('richText.clearFormat')"
        @click="emitRichTextCommand('clear')"
      ><i-icon-park-outline:format /></CheckboxButton>
      <CheckboxButton
        style="flex: 1;"
        :checked="!!textFormatPainter"
        v-tooltip="t('richText.formatPainter')"
      ...
      <Popover placement="bottom-end" trigger="click" v-model:value="linkPopoverVisible" style="width: 25%;">
        <template #content>
          <div class="link-popover">
            <Input v-model:value="link" :placeholder="t('richText.linkPlaceholder')" />
            <div class="btns">
              <Button size="small" :disabled="!richTextAttrs.link" @click="removeLink()" style="margin-right: 5px;">{{ t('richText.linkRemove') }}</Button>
              <Button size="small" type="primary" @click="updateLink(link)">{{ t('richText.linkConfirm') }}</Button>
            </div>
          </div>
        </template>
        <CheckboxButton
          ...
          v-tooltip="t('richText.hyperlink')"
    ...
    >
      <RadioButton value="left" v-tooltip="t('richText.alignLeft')" style="flex: 1;"><i-icon-park-outline:align-text-left /></RadioButton>
      <RadioButton value="center" v-tooltip="t('richText.alignCenter')" style="flex: 1;"><i-icon-park-outline:align-text-center /></RadioButton>
      <RadioButton value="right" v-tooltip="t('richText.alignRight')" style="flex: 1;"><i-icon-park-outline:align-text-right /></RadioButton>
      <RadioButton value="justify" v-tooltip="t('richText.alignJustify')" style="flex: 1;"><i-icon-park-outline:align-text-both /></RadioButton>
    </RadioGroup>
    ...
    <div class="row" passive>
      <ButtonGroup style="flex: 1;">
        <Button
          ...
          style="flex: 1;"
          v-tooltip="t('richText.bulletList')"
    ...
    <div class="row">
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="t('richText.indentReduce')" @click="emitRichTextCommand('indent', '-1')"><i-icon-park-outline:indent-left /></Button>
        <Popover trigger="click" v-model:value="indentLeftPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '-1')">{{ t('richText.indentFirstLineReduce') }}</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
      <div style="width: 10px;"></div>
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="t('richText.indentIncrease')" @click="emitRichTextCommand('indent', '+1')"><i-icon-park-outline:indent-right /></Button>
        <Popover trigger="click" v-model:value="indentRightPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '+1')">{{ t('richText.indentFirstLineIncrease') }}</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error(t('message.invalidLink'))
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error(t('message.noContent'))
