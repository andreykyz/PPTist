  ...
  <div class="theme-styles-extract">
    ...
    <div class="content">
      <div class="config-item" v-if="themeStyles.fontNames.length">
        <div class="label">{{ t('generic.fontFamily') }}：</div>
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontNames" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.fontName = index">{{ t('generic.select') }}</div>
              <div class="config-btn" @click="updateTheme({ fontName: item }); selectedIndex.fontName = index">{{ t('generic.applyAsTheme') }}</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.fontColors.length">
        <div class="label">{{ t('generic.fontColor') }}：</div>
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.fontColor = index">{{ t('generic.select') }}</div>
              <div class="config-btn" @click="updateTheme({ fontColor: item }); selectedIndex.fontColor = index">{{ t('generic.applyAsTheme') }}</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.backgroundColors.length">
        <div class="label">{{ t('generic.backgroundColorLabel') }}：</div>
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.backgroundColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.backgroundColor = index">{{ t('generic.select') }}</div>
              <div class="config-btn" @click="updateTheme({ backgroundColor: item }); selectedIndex.backgroundColor = index">{{ t('generic.applyAsTheme') }}</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">{{ t('generic.themeColorsLabel') }}：<span class="tip">{{ t('generic.excludeColors') }}</span></div>
    ...
    <div class="btns">
      <Button class="btn" type="primary" @click="updateAllThemes()"><i-icon-park-outline:check /> {{ t('generic.saveAsTheme') }}</Button>
    </div>
  </div>
</template>
...
const tabs: TabItem[] = [
  { key: 'single', label: t('themeStyleExtract.fromCurrent') },
  { key: 'all', label: t('themeStyleExtract.fromAll') },
]
...
const updateAllThemes = () => {
  ...
  if (themeColors.length > 6) {
    ...
    message.warning(t('generic.colorLimit'))
  }
