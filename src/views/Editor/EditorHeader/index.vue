<template>
  <div class="editor-header">
          ...
          <div class="main-menu">
            <div class="ai-menu" @click="openAIPPTDialog(); mainMenuVisible = false">
              ...
              <div class="aippt-content">
                ...
                <div class="aippt-subtitle">{{ t('header.aippt.subtitle') }}</div>
              </div>
            </div>
          </div>
          <Divider :margin="10" />
          <div class="import-section">
            ...
            <div class="import-grid">
              <FileInput class="import-block" accept="application/vnd.openxmlformats-officedocument.presentationml.presentation" @change="files => {
                ...
                <span class="sub-label">{{ t('header.import.test') }}</span>
              </FileInput>
              ...
              <FileInput class="import-block" accept=".pptist" @change="files => {
                ...
                <span class="sub-label">{{ t('header.import.exclusive') }}</span>
              </FileInput>
            </div>
          </div>
          <Divider :margin="10" />
          <PopoverMenuItem class="popover-menu-item" @click="setDialogForExport('pptx')"><i-icon-park-outline:download class="icon" /> {{ t('header.export.label') }}</PopoverMenuItem>
          <Divider :margin="10" />
          <PopoverMenuItem class="popover-menu-item" @click="resetSlides(); mainMenuVisible = false"><i-icon-park-outline:refresh class="icon" /> {{ t('header.reset') }}</PopoverMenuItem>
          <PopoverMenuItem class="popover-menu-item" @click="openMarkupPanel(); mainMenuVisible = false"><i-icon-park-outline:mark class="icon" /> {{ t('header.markup') }}</PopoverMenuItem>
          <PopoverMenuItem class="popover-menu-item" @click="mainMenuVisible = false; hotkeyDrawerVisible = true"><i-icon-park-outline:command class="icon" /> {{ t('header.shortcuts') }}</PopoverMenuItem>
    ...
    <div class="right">
      <div class="group-menu-item">
        ...
        <Popover trigger="click" center>
          <template #content>
            <PopoverMenuItem class="popover-menu-item" @click="enterScreeningFromStart()"><i-icon-park-outline:slide-two class="icon" /> {{ t('header.slideshowFromStart') }}</PopoverMenuItem>
            <PopoverMenuItem class="popover-menu-item" @click="enterScreening()"><i-icon-park-outline:ppt class="icon" /> {{ t('header.slideshowFromCurrent') }}</PopoverMenuItem>
          </template>
    ...
    >
      <HotkeyDoc />
      <template v-slot:title>{{ t('header.hotkeyDrawerTitle') }}</template>
    </Drawer>
