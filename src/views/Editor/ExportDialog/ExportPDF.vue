  ...
  <div class="export-pdf-dialog">
    ...
    <div class="configs">
      <div class="row">
        ...
        >
          <RadioButton style="width: 50%;" value="all">{{ t('export.all') }}</RadioButton>
          <RadioButton style="width: 50%;" value="current">{{ t('export.current') }}</RadioButton>
        </RadioGroup>
      </div>
      <div class="row">
        <div class="title">{{ t('export.perPage') }}</div>
        <Select 
          class="config-item"
          v-model:value="count"
          :options="[
    ...
    <div class="btns">
      <Button class="btn export" type="primary" @click="expPDF()"><i-icon-park-outline:download /> {{ t('export.buttonExportPDF') }}</Button>
